    ...
    if ($stmt = $pdo->prepare($query)) {
        ...
        if ($stmt->rowCount() > 0) {
            ...
            if ($hashed_password == $result["password"]){
                ...
                $_SESSION['user'] = serialize(new User($result["user_id"], $result["role_id"], $result["email"], $result["location_opt_in"], $result["leaderboard_opt_in"], $result["name"], $result["pronouns"], $result["file_loc"]));
